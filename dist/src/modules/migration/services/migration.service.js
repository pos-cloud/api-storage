...
const media_enum_1 = require("src/common/enums/media.enum");
const migration_image_function_1 = require("src/common/functions/migration-image.function");
const database_service_1 = require("src/database/services/database.service");
const upload_service_1 = require("src/modules/upload/services/upload.service");
let MigrationService = class MigrationService {
    constructor(databaseService, uploadService) {
        this.databaseService = databaseService;
        this.uploadService = uploadService;
    }
    async migrationImages(database) {
        await this.databaseService.initConnection(database);
        const collectionEmun = Object.values(media_enum_1.default);
        const promises = [];
    ...
    async arrayPromiseDocument(documents, database, nameDirectory, itemCollection, collection) {
        ...
        const promises = [];
        try {
            for (var _d = true, documents_1 = __asyncValues(documents), documents_1_1; documents_1_1 = await documents_1.next(), _a = documents_1_1.done, !_a; _d = true) {
